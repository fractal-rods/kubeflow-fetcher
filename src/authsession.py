...
def get_istio_auth_session(url: str, username: str, password: str) -> dict:
    ...
    WARNING: only Dex `staticPasswords` and `LDAP` authentication are currently
    supported (we default default to using `staticPasswords` if both are enabled)
    ...
    with requests.Session() as s:
        ...
        # else, we need to be redirected to the actual login page
            ...
            if resp.status_code != 200:
                raise RuntimeError(
                    f"HTTP status code '{resp.status_code}' for GET against: "
                    f"{redirect_url_obj.geturl()}"
                )
